		...
		if (data?.data.length > 0) {
			...
			for await(let error of data.data){
				...
				if (errorItemList?.length > 0){
					...
					const uuid = errObj.tags[0].fields.find(obj => obj.key === 'uuid')
					errorData.uuid = uuid
				} else {
					console.log(`Trace not contains any error messages. Invalid trace?`)
					continue
				}
				
				if (previousErros.get(errorData.uuid)) {
					console.log(`Found same UUID trace: ${errorData.uuid}, skip.`)
					continue
				}
				...
				previousErros.set(errorData.uuid)
			}
...
function isSameError(error){
	if (previousError) {
		if (
			...
			previousError.stack === error.stack) {
				console.log('Got the same stack trace, skip.')
				previousError = error
				return true
		}
	}
